...
public class EmailInboxFrameTester extends JFrame {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    public EmailInboxFrameTester() { this(null); }
 
    public EmailInboxFrameTester(String name) {
        this.currentUser = name;
 
        setTitle("Bug Tracker Pro — Email Inbox");
        setSize(860, 560);
    ...
    private void buildTopBar() {
        ...
        JLabel roleBadge = pillLabel("TESTER", EMAIL_CLR);
        roleBadge.setBounds(178, 17, 58, 18);
        bar.add(roleBadge);
        ...
        if (currentUser != null) {
            ...
            userLabel.setBounds(250, 18, 200, 16);
            bar.add(userLabel);
        }
        ...
        dateLabel.setBounds(460, 18, 160, 16);
        bar.add(dateLabel);
    ...
    private void buildEmailPanel() {
        ...
        // Column widths: From | To | Subject | Preview
    ...
    private void buildStatusBar() {
        ...
        JLabel right = new JLabel("Email Portal  ·  " +
            LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy")));
        right.setFont(FONT_SMALL);
        right.setForeground(TEXT_DIM);
        right.setBounds(680, 13, 160, 16);
        bar.add(right);
    }
    ...
    try (BufferedReader br = new BufferedReader(new FileReader("emails.txt"))) {
        ...
        while ((line = br.readLine()) != null) {
            ...
            if (line.startsWith("From:")) {
                ...
                addEmailIfMatchesUser(from, to, subject, message);
        ...
        // ✅ مهم: حفظ آخر إيميل لو مفيش --- في الآخر
        if (!to.isEmpty()) {
            addEmailIfMatchesUser(from, to, subject, message);
        }
    ...
    rowTimer.addActionListener(e -> {
        if (animatedRows < allEmails.size()) {
            ...
            model.addRow(new Object[]{
                email[0], email[1], email[2], preview
            });
...
private void addEmailIfMatchesUser(String from, String to, String subject, String message) {

    if (to == null || to.isEmpty()) return;

    // لو مفيش user → اعرض كل الإيميلات
    if (currentUser == null) {
    ...
    // نقارن صح
    if (to.equalsIgnoreCase(userEmail)) {
        allEmails.add(new String[]{
            from.isEmpty() ? "Unknown" : from,
            to, subject, message
        });
    ...
    private void drawBackground(Graphics g) {
        ...
        drawOrb(g2, (int)(getWidth()*0.10 + Math.cos(r)*50), (int)(getHeight()*0.22 + Math.sin(r)*30), 200, EMAIL_CLR, 22);
        drawOrb(g2, (int)(getWidth()*0.90 + Math.sin(r)*40), (int)(getHeight()*0.78 + Math.cos(r)*25), 180, EMAIL_CLR2, 20);
